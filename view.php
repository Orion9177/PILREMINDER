...
<form method="post" enctype="multipart/form-data" action="">
    <fieldset><legend>Ajout d'un rappel</legend>     
        ...
        value="<?php if(isset($_SESSION['reminder'])) {echo $_SESSION['reminder'];}?>" required>
        ...
        <input id="perimetre" list="required-perimetre" name="perimetre" required>
        <datalist id="required-perimetre">
            <option value = "Commun">
            <option value = "Distribué">
            <option value = "Mainframe">
        </datalist>

        <input id="env" name="env" type="hidden" value="<?php $env; ?>">
...
$perimetre = isset($_POST['perimetre']) ? $_POST['perimetre'] : NULL;
...
//Requête d'insertion en base du formulaire
        $req_add = "INSERT INTO $dbtable (name, creation_date, expiration, action, delay, statut, mail, perimetre) 
                    VALUE (:name, :creation_date, :expiration, :action, :delay, :statut, :mail, :perimetre)";
...
// Si l'expiration est dépassée, la BDD est mise à jour en changeant le statut 
    if($timediff <= '00:00:00' && $mail != 'oui'){
        ...
        SET statut = 'Expiré'
        WHERE TIMEDIFF (NOW(), expiration)>'00:00:00' AND statut LIKE 'En cours' AND NOT statut IN ('Traité', 'Annulé')";
...
//Envoi du mail de rappel au Pilotage BPCE    
		...
		$destinataire_commun = "BPCE-IT_PILOTAGE_ZOS_B@bpce-it.fr,BPCE-IT_PILOTAGE_DIS_B@bpce-it.fr";
        $destinataire_dis = "BPCE-IT_PILOTAGE_DIS_B@bpce-it.fr";
        $destinataire_z = "BPCE-IT_PILOTAGE_ZOS_B@bpce-it.fr";
		...
		$message = "<p>Bonjour,</p>
		<p>Le rappel <strong>".$name."</strong> de périmètre <strong>".$perimetre."</strong> a expiré.</p>
        ...
        if($perimetre = 'Commun'){
            mail($destinataire_commun,$sujet,$message,$headers);
        ...
        if($perimetre = 'Distribué'){
            mail($destinataire_dis,$sujet,$message,$headers);
        ...
        if($perimetre = 'Mainframe'){
            mail($destinataire_z,$sujet,$message,$headers);
...
$req_fetch_env = "SELECT id, name, action, statut, expiration, TIMEDIFF(expiration, NOW()), TIMEDIFF(NOW(), expiration), mail, perimetre
FROM $dbtable 
WHERE perimetre LIKE '$env'
AND statut IN ('En cours', 'Expiré') 
ORDER BY TIMEDIFF(expiration, NOW()) ASC";

$DB_fetch_env = $pdo->prepare($req_fetch_env);
$DB_fetch_env -> execute();

    while ($row=$DB_fetch_env->fetch(PDO::FETCH_ASSOC)) {
...
$id = isset($_POST['id']) ? $_POST['id'] : NULL;
        ...
        if (!empty($treated)) {
			...
			echo "<script language='Javascript'>document.location.href = 'view.php';</script>";
        }
